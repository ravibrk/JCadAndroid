...
import ormlite.core.dao.BaseForeignCollection;
import ormlite.core.dao.Dao;
import ormlite.core.dao.DaoManager;
import ormlite.core.dao.EagerForeignCollection;
import ormlite.core.dao.ForeignCollection;
import ormlite.core.dao.LazyForeignCollection;
import ormlite.core.dao.ObjectCache;
import ormlite.core.db.DatabaseType;
import ormlite.core.field.types.VoidType;
import ormlite.core.misc.SqlExceptionUtil;
import ormlite.core.stmt.mapped.MappedQueryForId;
...
/**
 * Per field information configured from the {@link DatabaseField} annotation and the associated {@link Field} in the class. Use the {@link #createFieldType} static method to
...
public class FieldType {
	...
	private MappedQueryForId<?, Object> mappedQueryForId;
	...
	public FieldType(ConnectionSource connectionSource, String tableName, Field field, DatabaseFieldConfig fieldConfig, Class<?> parentClass) throws SQLException {
		...
		if (fieldConfig.isForeign() || fieldConfig.isForeignAutoRefresh() || foreignColumnName != null) {
			...
			if (ForeignCollection.class.isAssignableFrom(clazz)) {
				throw new SQLException("Field '" + field.getName() + "' in class " + clazz + "' should use the @" + ForeignCollectionField.class.getSimpleName()
			...
			if (byte[].class.isAssignableFrom(clazz)) {
				...
				throw new SQLException("ORMLite does not know how to store " + clazz + " for field '" + field.getName() + "'.  Use another class, custom persister, or to serialize it use "
	...
	/**
	 * Because we go recursive in a lot of situations if we construct DAOs inside of the FieldType constructor, we have to do this 2nd pass initialization so we can better use the
	 * DAO caches.
	 * 
	...
	public void configDaoInformation(ConnectionSource connectionSource, Class<?> parentClass) throws SQLException {
		Class<? extends Model> fieldClass = (Class<? extends Model>) field.getType();
		DatabaseType databaseType = connectionSource.getDatabaseType();
		TableInfo<?> foreignTableInfo;
		final FieldType foreignIdField;
		final FieldType foreignFieldType;
		final BaseDaoImpl<? extends Model> foreignDao;
		final MappedQueryForId<? extends Model, Object> mappedQueryForId;

		String foreignColumnName = fieldConfig.getForeignColumnName();
		if (fieldConfig.isForeignAutoRefresh() || foreignColumnName != null) {
			DatabaseTableConfig<? extends Model> tableConfig = (DatabaseTableConfig<? extends Model>) fieldConfig.getForeignTableConfig();
			if (tableConfig == null) {
				// NOTE: the cast is necessary for maven
				foreignDao = (BaseDaoImpl<? extends Model>) DaoManager.createDao(connectionSource, fieldClass);
				foreignTableInfo = foreignDao.getTableInfo();
			} else {
				tableConfig.extractFieldTypes(connectionSource);
				// NOTE: the cast is necessary for maven
				foreignDao = (BaseDaoImpl<? extends Model>) DaoManager.createDao(connectionSource, tableConfig);
				foreignTableInfo = foreignDao.getTableInfo();
			}
			if (foreignColumnName == null) {
			...
			MappedQueryForId<?, Object> castMappedQueryForId = (MappedQueryForId<?, Object>) MappedQueryForId.build(databaseType, foreignTableInfo, foreignIdField);
			...
			if (tableConfig != null) {
				...
				/*
				 * Initially we were only doing this just for BaseDaoEnabled.class and isForeignAutoCreate(). But we need it also for foreign fields because the alternative was to
				 * use reflection. Chances are if it is foreign we're going to need the DAO in the future anyway so we might as well create it. This also allows us to make use of
				 * any table configs.
				 */
				// NOTE: the cast is necessary for maven
			...
			if (isForeignAutoCreate() && !foreignIdField.isGeneratedId()) {
				throw new IllegalArgumentException("Field " + field.getName() + ", if foreignAutoCreate = true then class " + fieldClass.getSimpleName()
			...
			Class<? extends Model> collectionClazz = (Class<? extends Model>) genericArguments[0];
			DatabaseTableConfig<? extends Model> tableConfig = (DatabaseTableConfig<? extends Model>) fieldConfig.getForeignTableConfig();
			BaseDaoImpl<? extends Model> foundDao;
			if (tableConfig == null) {
				BaseDaoImpl<? extends Model> castDao = (BaseDaoImpl<? extends Model>) DaoManager.createDao(connectionSource, collectionClazz);
				foundDao = castDao;
			} else {
				BaseDaoImpl<? extends Model> castDao = (BaseDaoImpl<? extends Model>) DaoManager.createDao(connectionSource, tableConfig);
				foundDao = castDao;
			}
			foreignDao = foundDao;
	...
	/**
	 * Return whether the field is an id field. It is an id if {@link DatabaseField#id}, {@link DatabaseField#generatedId}, OR {@link DatabaseField#generatedIdSequence} are
	 * enabled.
	 */
	public boolean isId() {
		return isId;
	...
	/**
	 * Return whether the field is a generated-id field. This is true if {@link DatabaseField#generatedId} OR {@link DatabaseField#generatedIdSequence} are enabled.
	 */
	...
	/**
	 * Return whether the field is a generated-id-sequence field. This is true if {@link DatabaseField#generatedIdSequence} is specified OR if {@link DatabaseField#generatedId} is
	 * enabled and the {@link DatabaseType#isIdSequenceNeeded} is enabled. If the latter is true then the sequence name will be auto-generated.
	 */
	...
	public void assignField(Object data, Object val, boolean parentObject, ObjectCache objectCache) throws SQLException {
		...
		if (foreignIdField != null && val != null) {
			...
			/*
			 * See if we don't need to create a new foreign object. If we are refreshing and the id field has not changed then there is no need to create a new foreign object and
			 * maybe lose previously refreshed field information.
			 */
			...
			if (cachedVal != null) {
				...
				if (levelCounters.autoRefreshLevel >= levelCounters.autoRefreshLevelMax) {
					...
					/*
					 * We may not have a mapped query for id because we aren't auto-refreshing ourselves. But a parent class may be auto-refreshing us with a level > 1 so we may
					 * need to build out query-for-id optimization on the fly here.
					 */
					if (mappedQueryForId == null) {
						MappedQueryForId<?, Object> castMappedQueryForId = (MappedQueryForId<?, Object>) MappedQueryForId.build(connectionSource.getDatabaseType(),
	...
	public boolean isComparable() throws SQLException {
		...
		/*
		 * We've seen dataPersister being null here in some strange cases. Why? It may because someone is searching on an improper field. Or maybe a table-config does not match the
		 * Java object?
		 */
		if (dataPersister == null) {
	 ...
	 * @param id
	 *            The id of the foreign object we will look for. This can be null if we are creating an empty collection.
	 */
	public <FT extends Model, FID> BaseForeignCollection<FT, FID> buildForeignCollection(Object parent, FID id) throws SQLException {
		// this can happen if we have a foreign-auto-refresh scenario
		if (foreignFieldType == null) {
			return null;
		}
		@SuppressWarnings("unchecked")
		Dao<FT> castDao = (Dao<FT>) foreignDao;
		...
		try {
			return new EagerForeignCollection<FT, FID>(castDao, parent, id, foreignFieldType, fieldConfig.getForeignCollectionOrderColumnName(),
					fieldConfig.isForeignCollectionOrderAscending());
		} finally {
			levelCounters.foreignCollectionLevel--;
		}
	...
	public <T> T resultToJava(DatabaseResults results, Map<String, Integer> columnPositions) throws SQLException {
		...
		if (fieldConfig.isForeign()) {
			/*
			 * Subtle problem here. If your foreign field is a primitive and the value was null then this would return 0 from getInt(). We have to specifically test to see if we
			 * have a foreign field so if it is null we return a null value to not create the sub-object.
			 */
	 ...
	 */
	public <T extends Model> int createWithForeignDao(T foreignData) throws SQLException {
		@SuppressWarnings("unchecked")
		Dao<T> castDao = (Dao<T>) foreignDao;
		return ((BaseDaoImpl<T>) castDao).create(foreignData);
	}
	...
	/**
	 * If we have a class Foo with a collection of Bar's then we go through Bar's DAO looking for a Foo field. We need this field to build the query that is able to find all Bar's
	 * that have foo_id that matches our id.
	 */
	...
	/**
	 * Configure our data persister and any dependent fields. We have to do this here because both the constructor and {@link #configDaoInformation} method can set the data-type.
	 */

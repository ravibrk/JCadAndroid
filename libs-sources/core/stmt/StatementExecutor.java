...
import ormlite.core.dao.RawRowObjectMapper;
...
import ormlite.core.misc.TransactionManager;
import ormlite.core.stmt.StatementBuilder.StatementType;
import ormlite.core.stmt.mapped.MappedCreate;
import ormlite.core.stmt.mapped.MappedDelete;
import ormlite.core.stmt.mapped.MappedDeleteCollection;
import ormlite.core.stmt.mapped.MappedQueryForId;
import ormlite.core.stmt.mapped.MappedRefresh;
import ormlite.core.stmt.mapped.MappedUpdate;
import ormlite.core.stmt.mapped.MappedUpdateId;
 ...
 * @author graywatson
 */
public class StatementExecutor<T extends Model, ID> implements GenericRowMapper<String[]> {
	...
	private final Dao<? extends Model> dao;
	 ...
	 */
	public StatementExecutor(DatabaseType databaseType, TableInfo<T> tableInfo, Dao<? extends Model> dao) {
	...
	public T queryForId(DatabaseConnection databaseConnection, ID id, ObjectCache objectCache) throws SQLException {
		if (mappedQueryForId == null) {
			mappedQueryForId = (MappedQueryForId<T, ID>) MappedQueryForId.build(databaseType, tableInfo, null);
		}
		return mappedQueryForId.execute(databaseConnection, id, (ObjectCache) objectCache);
	}
	...
	public T queryForFirst(DatabaseConnection databaseConnection, PreparedStmt<T> preparedStmt, ObjectCache objectCache) throws SQLException {
		...
		try {
			results = stmt.runQuery((ormlite.core.dao.ObjectCache) objectCache);
			if (results.first()) {
	 ...
	 */
	public SelectIterator<T, ID> buildIterator(BaseDaoImpl<T> classDao, ConnectionSource connectionSource,
	...
	public GenericRawResults<String[]> queryRaw(ConnectionSource connectionSource, String query, String[] arguments, ObjectCache objectCache) throws SQLException {
		...
		CompiledStatement compiledStatement = null;
		try {
			compiledStatement = connection.compileStatement(query, StatementType.SELECT, noFieldTypes, DatabaseConnection.DEFAULT_RESULT_FLAGS);
			assignStatementArguments(compiledStatement, arguments);
			GenericRawResults<String[]> rawResults = new RawResultsImpl<String[], T>(connectionSource, connection, query, String[].class, compiledStatement, this, objectCache);
			compiledStatement = null;
			connection = null;
			return rawResults;
		} finally {
		...
		try {
			compiledStatement =
					connection.compileStatement(query, StatementType.SELECT, noFieldTypes,
							DatabaseConnection.DEFAULT_RESULT_FLAGS);
			assignStatementArguments(compiledStatement, arguments);
			RawResultsImpl<UO, T> rawResults =
					new RawResultsImpl<UO, T>(connectionSource, connection, query, String[].class, compiledStatement,
		...
		try {
			compiledStatement =
					connection.compileStatement(query, StatementType.SELECT, noFieldTypes,
							DatabaseConnection.DEFAULT_RESULT_FLAGS);
			assignStatementArguments(compiledStatement, arguments);
			RawResultsImpl<Object[], T> rawResults =
					new RawResultsImpl<Object[], T>(connectionSource, connection, query, Object[].class,
	...
	public boolean ifExists(DatabaseConnection connection, ID id) throws SQLException {
		if (ifExistsQuery == null) {
			QueryBuilder<T, ID> qb = new QueryBuilder<T, ID>(databaseType, tableInfo, (Dao<T>) dao);
			qb.selectRaw("COUNT(*)");
	...
	private void prepareQueryForAll() throws SQLException {
		if (preparedQueryForAll == null) {
			preparedQueryForAll = new QueryBuilder<T, ID>(databaseType, tableInfo, (Dao<T>) dao).prepare();
		}
	}

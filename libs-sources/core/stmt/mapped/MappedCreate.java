...
import ormlite.core.support.GeneratedKeyHolder;
 ...
 * @author graywatson
 */
public class MappedCreate<T extends Model, ID> extends BaseMappedStatement<T, ID> {

	private final String queryNextSequenceStmt;
	private String dataClassName;
	private int versionFieldTypeIndex;

	private MappedCreate(TableInfo<T> tableInfo, String statement, FieldType[] argFieldTypes,
	...
	public int insert(DatabaseType databaseType, DatabaseConnection databaseConnection, T data, ObjectCache objectCache)
		...
		if (idField != null) {
			...
			if (idField.isSelfGeneratedId() && idField.isGeneratedId()) {
				if (assignId) {
					idField.assignField(data, idField.generateId(), false, (ormlite.core.dao.ObjectCache) objectCache);
				}
		...
		try {
			...
			if (tableInfo.isForeignAutoCreate()) {
				for (FieldType fieldType : tableInfo.getFieldTypes()) {
					...
					// get the field value
					T foreignObj = fieldType.extractRawJavaFieldValue(data);
			...
			int rowC;
			try {
				rowC = databaseConnection.insert(statement, args, argFieldTypes, (ormlite.core.support.GeneratedKeyHolder) keyHolder);
	...
	public static <T extends Model, ID> MappedCreate<T, ID> build(DatabaseType databaseType, TableInfo<T> tableInfo) {
	...
	private void assignIdValue(T data, Number val, String label, ObjectCache objectCache) throws SQLException {
		...
		idField.assignIdValue(data, val, (ormlite.core.dao.ObjectCache) objectCache);

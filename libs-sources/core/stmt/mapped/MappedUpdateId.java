 ...
 * @author graywatson
 */
public class MappedUpdateId<T extends Model, ID> extends BaseMappedStatement<T, ID> {

	private MappedUpdateId(TableInfo<T> tableInfo, String statement, FieldType[] argFieldTypes) {
			...
			if (rowC > 0) {
				if (objectCache != null) {
					...
					if (obj != null && obj != data) {
						...
						idField.assignField(obj, newId, false, (ormlite.core.dao.ObjectCache) objectCache);
					}
				}
				// adjust the object to assign the new id
				idField.assignField(data, newId, false, (ormlite.core.dao.ObjectCache) objectCache);
			}
	...
	public static <T extends Model, ID> MappedUpdateId<T, ID> build(DatabaseType databaseType, TableInfo<T> tableInfo)
			throws SQLException {

 ...
 * @author graywatson
 */
public class MappedUpdate<T extends Model, ID> extends BaseMappedStatement<T, ID> {

	private final FieldType versionFieldType;
	private final int versionFieldTypeIndex;

	private MappedUpdate(TableInfo<T> tableInfo, String statement, FieldType[] argFieldTypes, FieldType versionFieldType, int versionFieldTypeIndex) {
		super(tableInfo, statement, argFieldTypes);
		this.versionFieldType = versionFieldType;
		this.versionFieldTypeIndex = versionFieldTypeIndex;
	}

	public static <T extends Model, ID> MappedUpdate<T, ID> build(DatabaseType databaseType, TableInfo<T> tableInfo) throws SQLException {
					...
					if (cachedData != null && cachedData != data) {
						...
						for (FieldType fieldType : tableInfo.getFieldTypes()) {
							if (fieldType != idField) {
								fieldType.assignField(cachedData, fieldType.extractJavaFieldValue(data), false, (ormlite.core.dao.ObjectCache) objectCache);
							}
						}
					}
				}

...
import ormlite.core.stmt.mapped.MappedPreparedStmt;
 ...
 * @author graywatson
 */
public abstract class StatementBuilder<T extends Model, ID> {
	...
	public StatementBuilder(DatabaseType databaseType, TableInfo<T> tableInfo, Dao<T> dao, StatementType type) {
	...
	protected MappedPreparedStmt<T, ID> prepareStatement(Long limit) throws SQLException {
		...
		return new MappedPreparedStmt<T, ID>(tableInfo, statement, argFieldTypes, resultFieldTypes, selectArgs, (databaseType.isLimitSqlSupported() ? null : limit), type);
	}

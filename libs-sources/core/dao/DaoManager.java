...
import ormlite.core.table.DatabaseTable;
...
public class DaoManager {
	...
	private static Map<ClassConnectionSource, Dao<? extends Model>> classMap = null;
	private static Map<TableConfigConnectionSource, Dao<? extends Model>> tableConfigMap = null;
	...
	public synchronized static <D extends Dao<T>, T extends Model> D createDao(ConnectionSource connectionSource, Class<T> clazz)
			throws SQLException {
		if (connectionSource == null) {
			throw new IllegalArgumentException("connectionSource argument cannot be null");
		}
		ClassConnectionSource key = new ClassConnectionSource(connectionSource, clazz);
		Dao<T> dao = (Dao<T>) lookupDao(key);
		if (dao != null) {
			D castDao = (D) dao;
			return castDao;
		}
		...
		if (databaseTable == null || databaseTable.daoClass() == Void.class
			...
			Dao<T> daoTmp;
			if (config == null) {
				daoTmp = (Dao<T>) BaseDaoImpl.createDao(connectionSource, clazz);
			} else {
				daoTmp = (Dao<T>) BaseDaoImpl.createDao(connectionSource, config);
			}
			dao = daoTmp;
			...
			try {
				dao = (Dao<T>) daoConstructor.newInstance(arguments);
	 ...
	 */
	public synchronized static <D extends Dao<?>, T extends Model> D lookupDao(ConnectionSource connectionSource, Class<T> clazz) {
		if (connectionSource == null) {
			throw new IllegalArgumentException("connectionSource argument cannot be null");
		}
		ClassConnectionSource key = new ClassConnectionSource(connectionSource, clazz);
		Dao<? extends Model> dao = lookupDao(key);
		@SuppressWarnings("unchecked")
		D castDao = (D) dao;
		return castDao;
	}
	 ...
	 */
	public synchronized static <D extends Dao<T>, T extends Model> D createDao(ConnectionSource connectionSource, DatabaseTableConfig<T> tableConfig) throws SQLException {
		if (connectionSource == null) {
			throw new IllegalArgumentException("connectionSource argument cannot be null");
		}
	 ...
	 * null.
	 */
	public synchronized static <D extends Dao<?>, T extends Model> D lookupDao(ConnectionSource connectionSource,
	 ...
	 * </p>
	 */
	public static synchronized void registerDao(ConnectionSource connectionSource, Dao<? extends Model> dao) {
		if (connectionSource == null) {
			throw new IllegalArgumentException("connectionSource argument cannot be null");
		}
	 ...
	 * configuration.
	 */
	public static synchronized void unregisterDao(ConnectionSource connectionSource, Dao<? extends Model> dao) {
		if (connectionSource == null) {
			throw new IllegalArgumentException("connectionSource argument cannot be null");
		}
	 ...
	 */
	public static synchronized void registerDaoWithTableConfig(ConnectionSource connectionSource, Dao<? extends Model> dao) {
		if (connectionSource == null) {
			throw new IllegalArgumentException("connectionSource argument cannot be null");
		}
	...
	private static void addDaoToClassMap(ClassConnectionSource key, Dao<? extends Model> dao) {
		if (classMap == null) {
			classMap = new HashMap<ClassConnectionSource, Dao<? extends Model>>();
		}
		classMap.put(key, dao);
	}

	private static void removeDaoToClassMap(ClassConnectionSource key, Dao<? extends Model> dao) {
		if (classMap != null) {
			classMap.remove(key);
		}
	}

	private static void addDaoToTableMap(TableConfigConnectionSource key, Dao<? extends Model> dao) {
		if (tableConfigMap == null) {
			tableConfigMap = new HashMap<TableConfigConnectionSource, Dao<? extends Model>>();
		}
		tableConfigMap.put(key, dao);
	}

	private static Dao<? extends Model> lookupDao(ClassConnectionSource key) {
		if (classMap == null) {
			classMap = new HashMap<ClassConnectionSource, Dao<? extends Model>>();
		}
		Dao<? extends Model> dao = classMap.get(key);
		if (dao == null) {
			return null;
		} else {
			return dao;
	...
	private static Dao<? extends Model> lookupDao(TableConfigConnectionSource key) {
		if (tableConfigMap == null) {
			tableConfigMap = new HashMap<TableConfigConnectionSource, Dao<? extends Model>>();
		}
		Dao<? extends Model> dao = tableConfigMap.get(key);
		if (dao == null) {
			return null;
		} else {
			return dao;
	 ...
	 */
	private static <D, T extends Model> D createDaoFromConfig(ConnectionSource connectionSource, Class<T> clazz) throws SQLException {
		// no loaded configs
		if (configMap == null) {
			return null;
		}
	...
	@SuppressWarnings("unchecked")
	public static <D extends Dao<?>, T extends Model> D doCreateDao(ConnectionSource connectionSource,
		...
		Dao<? extends Model> dao = lookupDao(tableKey);
		if (dao != null) {
			D castDao = (D) dao;
			return castDao;
		}
		...
		if (databaseTable == null || databaseTable.daoClass() == Void.class
				|| databaseTable.daoClass() == BaseDaoImpl.class) {
			Dao<? extends Model> daoTmp = BaseDaoImpl.createDao(connectionSource, tableConfig);
			dao = daoTmp;
		} else {
			Class<?> daoClass = databaseTable.daoClass();
			...
			try {
				dao = (Dao<? extends Model>) constructor.newInstance(arguments);
			} catch (Exception e) {
				throw SqlExceptionUtil.create("Could not call the constructor in class " + daoClass, e);
			}
		}

...
import ormlite.core.field.ForeignCollectionField;
 ...
 * @param <Long>
 *            The class of the Long column associated with the class. The T class does not require an Long field. The class
 *            needs an Long parameter however so you can use Void or Object to satisfy the compiler.
 * @author graywatson
 */
public interface Dao<T extends Model> extends CloseableIterable<T> {
	...
	/**
	 * Retrieves an object associated with a specific Long.
	 * 
	 * @param id
	 *            Identifier that matches a specific row in the database to find and return.
	 * @return The object that has the Long field which equals id or null if no matches.
	 * @throws SQLException
	...
	/**
	 * This is a convenience method to creating a data item but only if the Long does not already exist in the table. This
	 ...
	 * 
	 * @return Either the data parameter if it was inserted (now with the Long field set via the create method) or the
	 ...
	 *             If there is only an Long field in the object. See the {@link #updateId} method.
	 */
	...
	/**
	 * Returns the Long from the data parameter passed in. This is used by some of the internal queries to be able to
	 * search by id.
	 */
	...
	/**
	 * Returns true if an object exists that matches this Long otherwise false.
	 */

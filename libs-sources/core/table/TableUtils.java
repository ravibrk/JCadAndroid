...
public class TableUtils {
	 ...
	 */
	public static <T extends Model> int createTable(ConnectionSource connectionSource, Class<T> dataClass) throws SQLException {
	 ...
	 */
	public static <T extends Model> int createTableIfNotExists(ConnectionSource connectionSource, Class<T> dataClass)
			throws SQLException {
	 ...
	 * @return The number of statements executed to do so.
	 */
	public static <T extends Model> int createTable(ConnectionSource connectionSource, DatabaseTableConfig<T> tableConfig)
			throws SQLException {
	 ...
	 * Create a table if it does not already exist. This is not supported by all databases.
	 */
	public static <T extends Model> int createTableIfNotExists(ConnectionSource connectionSource, DatabaseTableConfig<T> tableConfig)
			throws SQLException {
	 ...
	 */
	public static <MODEL extends Model, ID> List<String> getCreateTableStatements(ConnectionSource connectionSource, Class<MODEL> dataClass)
			throws SQLException {
		Dao<MODEL> dao = DaoManager.createDao(connectionSource, dataClass);
		if (dao instanceof BaseDaoImpl<?>) {
			return addCreateTableStatements(connectionSource, ((BaseDaoImpl<?>) dao).getTableInfo(), false);
		} else {
			TableInfo<MODEL> tableInfo = new TableInfo<MODEL>(connectionSource, null, dataClass);
	 ...
	 * @return The collection of table create statements.
	 */
	public static <MODEL extends Model, ID> List<String> getCreateTableStatements(ConnectionSource connectionSource,
			DatabaseTableConfig<MODEL> tableConfig) throws SQLException {
		Dao<MODEL> dao = DaoManager.doCreateDao(connectionSource, tableConfig);
		if (dao instanceof BaseDaoImpl<?>) {
			return addCreateTableStatements(connectionSource, ((BaseDaoImpl<?>) dao).getTableInfo(), false);
		} else {
			tableConfig.extractFieldTypes(connectionSource);
			TableInfo<MODEL> tableInfo = new TableInfo<MODEL>(connectionSource.getDatabaseType(), null, tableConfig);
			return addCreateTableStatements(connectionSource, tableInfo, false);
		}
	}
	 ...
	 * @return The number of statements executed to do so.
	 */
	public static <T extends Model, ID> int dropTable(ConnectionSource connectionSource, Class<T> dataClass, boolean ignoreErrors) throws SQLException {
		DatabaseType databaseType = connectionSource.getDatabaseType();
		Dao<T> dao = DaoManager.createDao(connectionSource, dataClass);
		if (dao instanceof BaseDaoImpl<?>) {
			return doDropTable(databaseType, connectionSource, ((BaseDaoImpl<T>) dao).getTableInfo(), ignoreErrors);
		} else {
			TableInfo<T> tableInfo = new TableInfo<T>(connectionSource, null, dataClass);
	 ...
	 * @return The number of statements executed to do so.
	 */
	public static <T extends Model, ID> int dropTable(ConnectionSource connectionSource, DatabaseTableConfig<T> tableConfig,
		...
		if (dao instanceof BaseDaoImpl<?>) {
			...
			tableConfig.extractFieldTypes(connectionSource);
			TableInfo<T> tableInfo = new TableInfo<T>(databaseType, null, tableConfig);
			return doDropTable(databaseType, connectionSource, tableInfo, ignoreErrors);
		}
	}
	...
	private static <T extends Model, ID> int createTable(ConnectionSource connectionSource, Class<T> dataClass, boolean ifNotExists) throws SQLException {
		Dao<T> dao = DaoManager.createDao(connectionSource, dataClass);
		if (dao instanceof BaseDaoImpl<?>) {
	...
	private static <T extends Model, ID> int createTable(ConnectionSource connectionSource, DatabaseTableConfig<T> tableConfig, boolean ifNotExists) throws SQLException {
		Dao<T> dao = DaoManager.createDao(connectionSource, tableConfig);
		if (dao instanceof BaseDaoImpl<?>) {
			return doCreateTable(connectionSource, ((BaseDaoImpl<?>) dao).getTableInfo(), ifNotExists);
		} else {
			tableConfig.extractFieldTypes(connectionSource);
			TableInfo<T> tableInfo = new TableInfo<T>(connectionSource.getDatabaseType(), null, tableConfig);
			return doCreateTable(connectionSource, tableInfo, ifNotExists);
		}
	}
	...
	private static <T extends Model, ID> int doDropTable(DatabaseType databaseType, ConnectionSource connectionSource, TableInfo<T> tableInfo, boolean ignoreErrors) throws SQLException {
	...
	private static <T extends Model, ID> void addDropIndexStatements(DatabaseType databaseType, TableInfo<T> tableInfo,
	 ...
	 */
	private static <T extends Model, ID> void addCreateTableStatements(DatabaseType databaseType, TableInfo<T> tableInfo,
	...
	private static <T extends Model, ID> void addCreateIndexStatements(DatabaseType databaseType, TableInfo<T> tableInfo,
	 ...
	 */
	private static <T extends Model, ID> void addDropTableStatements(DatabaseType databaseType, TableInfo<T> tableInfo,
			List<String> statements) {
		List<String> statementsBefore = new ArrayList<String>();
		List<String> statementsAfter = new ArrayList<String>();
		for (FieldType fieldType : tableInfo.getFieldTypes()) {
	...
	private static <T extends Model, ID> int doCreateTable(ConnectionSource connectionSource, TableInfo<T> tableInfo,
	...
	private static <T extends Model, ID> List<String> addCreateTableStatements(ConnectionSource connectionSource,
			TableInfo<T> tableInfo, boolean ifNotExists) throws SQLException {
		List<String> statements = new ArrayList<String>();
		List<String> queriesAfter = new ArrayList<String>();
